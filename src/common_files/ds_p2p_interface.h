...
typedef struct {
    ...
    uint32_t timestamp;
    // NOTE: phryctoria
    // timestamp2 for sender
    // timestamp2_rx for receiver 
    uint32_t timestamp2_tx;    
    uint32_t timestamp2_rx;
    Position position;
} copter_full_state_t;
...
extern void broadcastRemoteRoot(Event *rightroot, int repeat);

extern void broadcastToken(Token *token, int repeat);

extern void broadcastSatCut(Satcut *s);
